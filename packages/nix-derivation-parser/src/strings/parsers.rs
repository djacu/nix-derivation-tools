//! This example shows an example of how to parse an escaped string. The rules for
//! the string are similar to JSON and rust. A string is:
//!
...
//!
//! * an escape followed by whitespace consumes all whitespace between the escape and
...
where
    ...
    // with a looping parser (like Repeat::fold), be sure that the loop won't
...
fn parse_literal<'a, E: ParserError<&'a str>>(input: &mut &'a str) -> PResult<&'a str, E> {
    ...
    // parser. The verification function accepts the output only if it returns true.
    // In this case, we want to ensure that the output of take_till is non-empty.
    not_quote_slash.verify(|s: &str| !s.is_empty()).parse_next(input)
}

// parser combinators are constructed from the bottom up: first we write parsers
// for the smallest elements (escaped characters), then combine them into larger
// parsers.
...
where
    ...
    // char with from_u32.
    parse_u32.verify_map(from_u32).parse_next(input)
}

/// Parse a backslash, followed by any amount of whitespace. This is used later to
/// discard any escaped whitespace.
fn parse_escaped_whitespace<'a, E: ParserError<&'a str>>(input: &mut &'a str) -> PResult<&'a str, E> {
