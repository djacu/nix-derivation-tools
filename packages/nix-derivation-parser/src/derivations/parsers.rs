...
fn parse_derivation_inputs(input: &str) -> IResult<&str, HashMap<PathBuf, DerivationInput>> {
    delimited(
        tag("["),
        fold_many1(
            tuple((parse_derivation_input, opt(tag(",")))),
            HashMap::new,
            |mut map, ((key, value), _)| {
                map.insert(key, value);
                map
...
mod tests {
    ...
    fn derivation_inputs_shadow() {
        assert_eq!(
            parse_derivation_inputs(concat!(
                r#"["#,
            ...
            Ok((
                "",
                HashMap::from([
                    ( PathBuf::from("/nix/store/2a4nqx30swmddxgd5f3y1h8gynwb1mp9-bison-3.8.2.drv"), DerivationInput { value: vec!["out".to_string()] } ),
                    ( PathBuf::from("/nix/store/81l2lyg7hx6zwlb7yamncj1b2pbz5rj1-tcb-1.2.drv"), DerivationInput { value: vec!["dev".to_string()] } ),
                    ( PathBuf::from("/nix/store/9jqhmw0ksi0gab01asfd8gfj3wv3ahg6-docbook-xsl-nons-1.79.2.drv"), DerivationInput { value: vec!["out".to_string()] } ),
                    ( PathBuf::from("/nix/store/9vyx2lhbiq2c6jg6xz68whkl29qy60j2-autoreconf-hook.drv"), DerivationInput { value: vec!["out".to_string()] } ),
                    ( PathBuf::from("/nix/store/b5gkdv8336qp2wx7qppmd54nl29y0zh4-libxcrypt-4.4.36.drv"), DerivationInput { value: vec!["out".to_string()] } ),
                    ( PathBuf::from("/nix/store/d4rparlxpipwi3y717ijj917h0lbmrbj-glibc-2.39-52.drv"), DerivationInput { value: vec!["bin".to_string()] } ),
                    ( PathBuf::from("/nix/store/dgj37ph9745jy0bnzfz2hl1x8yjhaawy-itstool-2.0.7.drv"), DerivationInput { value: vec!["out".to_string()] } ),
                    ( PathBuf::from("/nix/store/hl008qyglrzrsyg59pc499jxaf1rvgjz-source.drv"), DerivationInput { value: vec!["out".to_string()] } ),
                    ( PathBuf::from("/nix/store/hll9cxnh7mm2maiy06vbxl6zk2y65kvh-fix-implicit-getdef_bool.patch.drv"), DerivationInput { value: vec!["out".to_string()] } ),
                    ( PathBuf::from("/nix/store/icld2xsizd7xabkfr396chagxcv7qaal-libxslt-1.1.39.drv"), DerivationInput { value: vec!["dev".to_string()] } ),
                    ( PathBuf::from("/nix/store/kblxy5ggi81bli1vkz550vpvmy36wlbp-linux-pam-1.6.1.drv"), DerivationInput { value: vec!["out".to_string()] } ),
                    ( PathBuf::from("/nix/store/lzc3r3m5yp5xj9qnbz56zrkq94d5hhsy-flex-2.6.4.drv"), DerivationInput { value: vec!["out".to_string()] } ),
                    ( PathBuf::from("/nix/store/nz98jzc49vlkky3vpq5lwjxh94b207fh-pkg-config-wrapper-0.29.2.drv"), DerivationInput { value: vec!["out".to_string()] } ),
                    ( PathBuf::from("/nix/store/pfkmysygw53mz830rhwfkadnzdxv96yw-libxml2-2.12.7.drv"), DerivationInput { value: vec!["dev".to_string()] } ),
                    ( PathBuf::from("/nix/store/wkgn8l6fyq3avhcpw1caj2r1z9dsw4r0-docbook-xml-4.5.drv"), DerivationInput { value: vec!["out".to_string()] } ),
                    ( PathBuf::from("/nix/store/wql9zbydwdr0nqxkm20crcbhn68wb4pc-stdenv-linux.drv"), DerivationInput { value: vec!["out".to_string()] } ),
                    ( PathBuf::from("/nix/store/xzz7s4cc4bakhaavx3qyn10sl9w7x445-libbsd-0.11.8.drv"), DerivationInput { value: vec!["dev".to_string()] } ),
                    ( PathBuf::from("/nix/store/ysv6wz83jkvg7d65j0js4bml9k0yc4sv-bash-5.2p32.drv"), DerivationInput { value: vec!["out".to_string()] } ),
                ])
            ))
        )
    }
